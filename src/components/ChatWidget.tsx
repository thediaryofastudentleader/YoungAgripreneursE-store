...
export default function ChatWidget({ orderId, popTrigger }: { orderId: string; popTrigger?: string }) {
  ...
  useEffect(() => {
    ...
    // Dedupe by id: the realtime echo of our own message must not duplicate
    // the optimistic local append done in handleSend.
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    const content = text;
    setText('');
    // B8: append our own message immediately — don't wait for the realtime echo
    const sent = await sendMessage(orderId, 'customer', app.user?.username || 'Customer', content, app.user?.id);
