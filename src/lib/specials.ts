...
export async function setSpecial(productId: string, discountPercent: number, createdBy?: string) {
  ...
  // created_by is a UUID FK to profiles — never send labels like 'dr-vogue'
  // (Postgres rejects them with a 400 invalid-uuid error).
  const row: Record<string, unknown> = {
    product_id: productId,
    discount_percent: discountPercent,
    active: true,
  };
  if (createdBy && UUID_RE.test(createdBy)) row.created_by = createdBy;
  const { data, error } = await supabase
    .from('product_specials')
