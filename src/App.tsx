...
import { sendOrderConfirmationEmail, sendOrderReviewRequestEmail } from '@/lib/email';
...
export default function App() {
  ...
  const addToCart = useCallback((product: Product, size?: string) => {
    setCart(prev => {
      ...
      if (existing) return prev.map(c => (c.id === product.id && c.selectedSize === size) ? { ...c, qty: c.qty + 1 } : c);
      return [...prev, { ...product, qty: 1, selectedSize: size }];
    });
    showToast(`${product.title}${size ? ` (${size})` : ''} added to cart`, 'success');
  }, [showToast]);
  ...
  const updateQty = useCallback((id: string, delta: number, size?: string) => {
    setCart(prev => {
      const item = prev.find(c => c.id === id && c.selectedSize === size);
      if (!item) return prev;
      const newQty = item.qty + delta;
      if (newQty <= 0) return prev.filter(c => !(c.id === id && c.selectedSize === size));
      return prev.map(c => (c.id === id && c.selectedSize === size) ? { ...c, qty: newQty } : c);
    });
  }, []);
...
const register = useCallback(async (username: string, email: string, phone: string, address: string, password: string): Promise<boolean> => {
  ...
  const cancelAccountDelete = useCallback(async () => {
    if (!user) return;
    // Must be `null` — Supabase silently drops `undefined` fields,
    // which previously left the deletion flag set forever.
    await updateProfile({ delete_requested_at: null });
  ...
  const finalizeOrder = useCallback(async (formData: FormData): Promise<string | null> => {
    ...
    const { error } = await supabase.from('orders').insert({
      ...
      address: formData.address,
      location: formData.location,
      items: cart.map(i => ({ id: i.id, title: i.title, qty: i.qty, price: i.price, image: i.image, selectedSize: i.selectedSize })),
      subtotal,
      delivery_fee: deliveryFee,
      total,
      payment_method: formData.pay,
    ...
    // Fire-and-forget: Brevo confirmation to the customer + formal
    // "register call" review request to the CEO (CEO only, spam-safe).
    sendOrderConfirmationEmail(order);
    sendOrderReviewRequestEmail(order);
