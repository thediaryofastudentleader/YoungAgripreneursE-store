            ...
            <div ref={scrollRef} className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide snap-x">
              {categories.map(cat => (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-4 py-3 rounded-2xl border-none cursor-pointer whitespace-nowrap snap-start transition-all ${
    ...
    <div
      ...
      <div className="p-3">
        ...
        <div className="flex items-center justify-between">
          <span className={`text-xs ${product.stock > 0 ? 'text-emerald-500' : 'text-red-500'} font-medium`}>
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      <div
        ...
        className={`w-full max-w-3xl max-h-[90vh] overflow-auto rounded-3xl shadow-2xl ${app.dark ? 'bg-slate-800' : 'bg-white'}`}
      >
        <div className="grid md:grid-cols-2">
          {/* Image */}
          <div className={`relative ${app.dark ? 'bg-slate-900' : 'bg-slate-100'} flex items-center justify-center min-h-[280px]`}>
            <img
              src={product.image}
              alt={product.title}
            ...
            />
            {product.originalPrice && (
              <span className="absolute top-3 left-3 px-2.5 py-1 rounded-lg bg-gradient-to-r from-amber-500 to-orange-500 text-white text-xs font-bold shadow-lg">
          ...
          <div className="p-6 flex flex-col">
            <div className="flex items-start justify-between gap-2 mb-1">
              <p className="text-xs font-bold uppercase tracking-wide text-emerald-500">{categoryLabel}</p>
              <button onClick={onClose} className={`p-1.5 rounded-full ${app.dark ? 'hover:bg-slate-700' : 'hover:bg-slate-100'}`}><X size={20} /></button>
            </div>
            <h3 className="font-bold text-xl leading-snug mb-3">{product.title}</h3>

            <div className="flex items-center gap-3 mb-3">
              <span className="font-extrabold text-2xl text-emerald-600">{formatPrice(product.price)}</span>
              {product.originalPrice && (
                <span className={`text-sm line-through ${app.dark ? 'text-slate-500' : 'text-slate-400'}`}>{formatPrice(product.originalPrice)}</span>
              )}
            </div>
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl border text-sm font-bold transition-all ${
                        selectedSize === size
                          ...
                          : app.dark ? 'border-slate-600 hover:border-emerald-500' : 'border-slate-300 hover:border-emerald-500'
                      }`}
                    >
                      {size}
                    </button>
            ...
            <div className="flex items-center gap-3 mb-5">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setQtyLocal(q => Math.max(1, q - 1))} className={`w-8 h-8 rounded-full flex items-center justify-center ${app.dark ? 'bg-slate-700' : 'bg-slate-200'}`}><Minus size={14} /></button>
                <span className="w-8 text-center font-bold">{qty}</span>
                <button onClick={() => setQtyLocal(q => Math.min(product.stock || 99, q + 1))} className="w-8 h-8 rounded-full bg-emerald-500 text-white flex items-center justify-center"><Plus size={14} /></button>
              </div>
            </div>

            {/* Description + structured facts */}
            <div className={`space-y-4 text-sm mb-6 ${app.dark ? 'text-slate-300' : 'text-slate-600'}`}>
              {details.features.length > 0 && (
                <div>
                  <p className="font-bold mb-1.5 text-emerald-500">Key Features</p>
              ...
              {details.howToUse.length > 0 && (
                <div>
                  <p className="font-bold mb-1.5 text-emerald-500">How to Use</p>
                  <ul className="list-disc pl-5 space-y-1">
              ...
              {details.suitableFor.length > 0 && (
                <div>
                  <p className="font-bold mb-1.5 text-emerald-500">Suitable For</p>
                  <ul className="list-disc pl-5 space-y-1">
              ...
              {details.sizeVolume && (
                <div className="flex gap-2">
                  <p className="font-bold text-emerald-500">Size/Volume:</p>
            ...
            <div className="mt-auto flex gap-2">
              <button
                onClick={handleAdd}
                disabled={product.stock <= 0}
                className="flex-1 py-4 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-bold shadow-lg shadow-emerald-500/30 disabled:opacity-50"
              >
                Add to Cart — {formatPrice(product.price * qty)}
              </button>
              <button
                onClick={() => app.toggleLike(product.id)}
                className={`w-14 rounded-2xl border flex items-center justify-center ${app.dark ? 'border-slate-600' : 'border-slate-300'}`}
              >
