...
import {
  ...
  LogIn, LogOut, User, MapPin, Phone, Mail, Lock, AlertTriangle,
  Clock, Eye, EyeOff, HelpCircle, ScrollText, ArrowRight
} from 'lucide-react';
...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Register Modal */}
      {app.showRegister && <RegisterModal />}
...
function LoginModal() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const success = app.login(email, password);
    ...
    <div className="fixed inset-0 z-[200] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className={`w-full max-w-sm ${app.dark ? 'bg-slate-800' : 'bg-white'} rounded-3xl p-6 shadow-2xl`}>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <button type="submit" className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-bold">
            Login
          </button>
        </form>
...
function RegisterModal() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const success = app.register(username, email, phone, address, password);
