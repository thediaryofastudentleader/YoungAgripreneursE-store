...
export default function TrackerPage() {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      ...
      const localOrders = JSON.parse(localStorage.getItem('yaf_orders') || '[]');
      const localOrder = localOrders.find((o: any) => o.order_id === orderId);
      if (localOrder) { setOrder(localOrder); setLoading(false); }
        ...
        <div className={`rounded-3xl p-6 mb-6 ${app.dark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} border shadow-sm`}>
          ...
          <div className="relative pl-8">
            ...
            {STATUS_STEPS.map((step, i) => {
              ...
              const historyItem = history.find((h: any) => h.status === step.id);
