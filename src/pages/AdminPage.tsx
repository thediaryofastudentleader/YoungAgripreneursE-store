...
export default function AdminPage() {
  ...
  const approveVogueDraft = async (draft: VogueDraft) => {
    if (!supabase) return;
    try {
      // created_by must be a real profiles UUID — the vogue_drafts row
      // already records that this special came from Dr. Vogue.
      await setSpecial(draft.product_id, draft.discount_percent, app.user?.id);
